...
public:
	...
	OrthographicParameterProjection(std::vector<cv::Vec2f> image_points, std::vector<cv::Vec3f> model_points, int width, int height) : Functor<double>(6, image_points.size()), image_points(image_points), model_points(model_points), width(width), height(height) {};
