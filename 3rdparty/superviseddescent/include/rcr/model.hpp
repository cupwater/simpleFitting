...
detection_model load_detection_model(std::string filename)
{
	...
	std::ifstream filex(filename, std::ios::binary);
	cereal::BinaryInputArchive input_archive(filex);
...
{
	std::ofstream filex(filename, std::ios::binary);
	cereal::BinaryOutputArchive output_archive(filex);
